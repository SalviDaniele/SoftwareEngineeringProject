...
/**
 * Indicates the second phase of the turn, when the player has to draw a card from either one of the decks
 * or the table. The phase validates the command entered by the player.
 */
@JsonAutoDetect(fieldVisibility = JsonAutoDetect.Visibility.ANY)
public class DrawPhase extends Phase implements Serializable {


    /**
     * The constructor of the draw phase class.
     *
     * @param match is the reference to the current match
     */
    public DrawPhase(MatchController match) {
        super(match);
    }


    /**
     * Empty constructor of the DrawPhase class, used for Jackson serialization.
     */
    public DrawPhase() {}
    ...
    public boolean processCommand(String command) {
        if (command.equalsIgnoreCase("draw")) {
            this.transition(new PlacingPhase(match));
            return true;
